...
ii <- seq_len(n)
  ...
  clus_fun_par <- function(x, kmax) foreach(i = 1:(kmax), .combine = cbind, .export = 'W.k', .packages = 'cluster') %dopar% (W.k(x, k = i))
  
  
#  cluster::clara(x, k = i)$silinfo$avg.width
  ...
  for(i in 1:length(boot_dat_clus)){
    gap_stats[[i]] <- as.numeric(boot_dat_clus[[i]]) - as.numeric(true_dat_clus) 
  }
...
print(dim(gap_stats))
  if(plot_clustering){
    ...
    plot(1:(ncol(gap_stats) ), gap_stats[1, ], pch = 20, ylim = c(min(gap_stats), max(gap_stats)), ylab = 'Gap', xlab = 'k', col = 'blue') #col = rgb(0, 0, 1, 0.3))
    for(i in 2:nrow(gap_stats)){
      points(jitter(1:(ncol(gap_stats) )), gap_stats[i, ], pch = 20, col = 'blue')# col = rgb(0, 0, 1, 0.5))
    }
    lines(1:(ncol(gap_stats) ), mean_gaps, col = 'red', lwd = 2)
  }
