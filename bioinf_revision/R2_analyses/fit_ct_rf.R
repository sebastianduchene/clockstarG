...
class_data_merge <- cbind.data.frame(as.data.frame(class_data_raw, stringsAsFactors = F), as.data.frame(clustering_r2[match(class_data_raw[, 1], rownames(clustering_r2)) ]))


class_data_merge <- data.frame(class_data_merge[which(!is.na(class_data_merge[, 2])), ], stingsAsFactors = F)
...
#clean_data_vars$t_l <- levels(clean_data_vars$t_length)[as.numeric(clean_data_vars$t_length)]
