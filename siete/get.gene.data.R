library(rentrez)
gene.data <- read.table("gene.names.csv", sep = ",", head = T, as.is = T)

gene.data[, 2] <- gsub(" ", "", gene.data[, 2])




cat("name,id,type,taxname,locus,description,location,family", file = "gene_data_test.txt", sep = "\n")

data.tags <- c("geneid", "gene_type", "taxname", "ref_locus", "ref_desc", "chromosome.*HuRef", "[Ff]amily")

n.dat.run <- nrow(gene.data)

n.dat.comp <- as.numeric(gsub("[A-Z]|[a-z]|[.]|[_]| ", "", system("wc -l gene_data_test.txt", intern = T)))


error.rep <- 0

while((n.dat.comp-1 < n.dat.run) && (time.run < 10)){      
      print(paste("I am searching for the genes in NCBI for gene", gene.data[n.dat.comp, 1], "with name", gene.data[n.dat.comp, 2]))
      gene_temp_search <- try(entrez_search(db = "Gene", term = paste0(gene.data[n.dat.comp, 2], "[Gene]"), usehistory = "y"))
if(class(gene_temp_search) != "try-error"){
      cookie <- gene_temp_search$WebEnv
      qk <- gene_temp_search$QueryKey
      gene_temp_data <- entrez_fetch(db = "Gene", WebEnv = cookie,  query_key = qk, rettype = "xml", ret.max = 1)
      gene_temp_data <- strsplit(gene_temp_data, "\n")[[1]]
      gene_temp_data_tags <- sapply(data.tags, function(x) grep(x, gene_temp_data, value = T)[1])

      # regexp to remove html tags
      gene_temp_data_tags <- gsub("(<([A-Z]|[a-z]|_|[-])+>)|(</([A-Z]|[a-z]|_|[-])+>)|^ +", "", gene_temp_data_tags)

      print(paste("The metadata for", gene.data[n.dat.comp, 2]))
      print(gene_temp_data_tags)
      cat(paste(c(paste0("gene", gene.data[n.dat.comp, 1]), gene_temp_data_tags), collapse = ","), file = "gene_data_test.txt", sep = "\n", append = T)      
      rm(list = c("gene_temp_search", "gene_temp_data", "gene_temp_data_tags"))
      error.rep <- 0
}else{
	error.rep <- error.rep + 1
	
	if(error.rep >= 10){
	 break
	}
	
}

      n.dat.comp <- as.numeric(gsub("[A-Z]|[a-z]|[.]|[_]| ", "", system("wc -l gene_data_test.txt", intern = T)))
      
}

if(error.rep >= 10){
	    print(paste("I tried over 10 times to extract the data for", gene.data[n.dat.comp, 2]))
	    
}


